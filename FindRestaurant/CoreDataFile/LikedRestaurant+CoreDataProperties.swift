...
extension LikedRestaurant {
    ...
    func convertToRestaurant() -> LikedRestaurantModel{
        return LikedRestaurantModel(name: self.name, address: self.address, photoReference: self.photoReference, distance: self.distance, rating: self.rating, openNow: self.openNow, id: self.id)
